...
// }
//
// abstract class CancellableCompleter<T> implements Completer<T> {
//   factory CancellableCompleter([Cancellable? cancellable]) =>
//       new _AsyncCompleter<T>(Cancellable().bindCancellable(cancellable));
//
//   factory CancellableCompleter.sync([Cancellable? cancellable]) =>
//       new _SyncCompleter<T>(Cancellable().bindCancellable(cancellable));
//
//   void cancel([dynamic reason]);
//
//   @override
//   CancellableFuture<T> get future;
// }
//
// abstract class _Completer<T> implements CancellableCompleter<T> {
//   final Cancellable cancellable;
//   late final _Future<T> future = new _Future<T>(cancellable);
//
//   _Completer(this.cancellable) {
//     cancellable.onCancel.then(
//         (error) => _completeError(error, AsyncError.defaultStackTrace(error)));
//   }
//
//   void cancel([dynamic reason]) {
...
//   _AsyncCompleter(super.cancellable);
//
//   void _complete([FutureOr<T>? value]) {
//     if (!future._mayComplete) throw new StateError("Future already completed");
...
//   _SyncCompleter(super.cancellable);
//
//   void _complete([FutureOr<T>? value]) {
//     if (!future._mayComplete) throw new StateError("Future already completed");
...
// }
//
// class _Future<T> extends CancellableFuture<T> {
//   final Cancellable cancellable;
//   final Zone _zone;
//
//   _Future(this.cancellable) : _zone = Zone.current;
//
//   _Future.immediate(FutureOr<T> result, this.cancellable)
//       : _zone = Zone.current {
//     _asyncComplete(result);
//   }
//
//   /// Creates a future with the value and the specified zone.
//   _Future.zoneValue(T value, this._zone, this.cancellable) {
//     _setValue(value);
//   }
//
//   _Future.immediateError(var error, StackTrace stackTrace, this.cancellable)
//       : _zone = Zone.current {
//     _asyncCompleteError(error, stackTrace);
//   }
//
//   /// Creates a future that is already completed with the value.
//   _Future.value(T value, Cancellable cancellable)
//       : this.zoneValue(value, Zone.current, cancellable);
//
...
//
//   @override
//   CancellableFuture<R> then<R>(FutureOr<R> f(T value), {Function? onError}) {
...
//     }
//     _Future<R> result =
//         new _Future<R>(cancellable.makeCancellable(infectious: true));
//     _addListener(new _FutureListener<T, R>.then(result, f, onError));
//     return result;
//   }
//
//   CancellableFuture<T> catchError(Function onError,
//       {bool test(Object error)?}) {
//     _Future<T> result = new _Future<T>(cancellable);
...
//   }
//
//   CancellableFuture<T> whenComplete(dynamic action()) {
//     _Future<T> result = new _Future<T>(cancellable);
//     if (!identical(result._zone, _rootZone)) {
...
//     _Future<T> _future = new _Future<T>(cancellable);
//     Timer timer;
//     if (onTimeout == null) {
...
// }
//
// unsafeCast<T>(dynamic value) => value as T;
//
